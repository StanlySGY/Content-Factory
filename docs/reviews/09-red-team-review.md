...
- [x] 注入风险：Prompt 注入 / 外部内容注入信任边界（RT-001）
- [x] 人工确认防伪与绑定（RT-002）
- [x] 审计完整性：防篡改 / 防删除技术控制（RT-003）
- [x] 服务身份与凭证最小化 / 轮换（RT-004）
- [x] 第三方插件供应链与运行时隔离（RT-005）
- [x] 跨项目隔离强制点（RT-006）
- [x] 敏感数据传播控制与脱敏管道（RT-007/008）
- [x] WSL 路径沙箱逃逸边界（RT-009）
- [x] 远端 MCP 传输安全（RT-010）
...
| RT-007 | Minor | 数据保护 | context_packs.sensitivity_level 脱敏靠写入方自觉，sensitive 上下文到 Provider（含外部 Codex/Gemini）的传播控制未定义 | db §5.8/§9.3 | 已修复 |
| RT-008 | Minor | 数据保护 | input_digest/脱敏无算法与不可逆要求，敏感值可能在摘要中残留 | mcp §9.2/§9.3, db §5.17 | 已修复 |
| RT-009 | Minor | 沙箱逃逸 | WSL 路径转换与工作目录沙箱交叉处（符号链接、`..`、`/mnt/c`、UNC）规范化与逃逸防护未明确 | agent §12.2/§9.4 | 已修复 |
| RT-010 | Minor | 传输安全 | 远端/HTTP/SSE MCP 未要求 TLS 与服务端身份校验，远端调用可能明文传输 | mcp §5.2/§6.1, db §5.13 | 已修复 |
...
| 2026-06-03 | 修复跟踪 | 批次 20 修复 | RT-007~010（4 Minor）→ 已修复；sensitivity 传播矩阵/digest 不可逆脱敏/WSL 路径沙箱/远端 TLS；RT 域全清；详见 fix-log 批次 20 |
