...
- [x] 工作流与阶段状态流转图自洽，且与 DB §8 一致（WF-001/002）
- [x] 回滚机制覆盖阶段、资产、配置，血缘语义清晰（WF-003/004/009）
- [x] 多 Agent 并行汇聚（join）与上下文合并机制明确（WF-005）
- [x] 数据映射与数据库设计一致（WF-006/008）
- [x] 阶段集与架构抽象一致（WF-007）
...
| WF-007 | Minor | 一致性 | §2 九阶段与架构 §8.2 抽象骨架命名/粒度不同，无映射说明 | wf §2 ↔ arch §8.2 | 已修复 |
| WF-008 | Minor | 一致性 | 细粒度 asset_type 词表与 DB §5.9 示例枚举对不上；review/publish 已独立表非 asset | wf §3 ↔ db §5.9 | 已修复 |
| WF-009 | Minor | 完整性 | §5.1 配置回滚未引用既有版本化机制（workflow_version / *_config_versions / profile_snapshot） | wf §5.1 | 已修复 |
| WF-010 | Minor | 完整性 | §4.1 取消(cancelled)仅部分阶段有出边，中间态可否取消未说明 | wf §4.1 | 已修复 |
...
| 2026-06-03 | 修复跟踪 | 批次 17 修复 | WF-007~010（4 Minor）→ 已修复；九阶段映射/asset_type 对齐/配置回滚版本/取消态；WF 域全清；详见 fix-log 批次 17 |
