...
1. **Writeback Executor Registration Contract Disabled Harness**：定义真实 writeback executor registry/descriptor contract，要求 feature flag 与 preflight matrix 通过后才允许注册；当前仍 `registered=false` / `executable=false`。
2. **Relay 真实回写 spike**：在 readiness handler、lease、writeback ledger、guard、transaction plan、apply guard、transaction prototype、transaction port、state transition policy 与 subject snapshot readiness 基础上实现单一 subject 类型的幂等 control-plane writeback，经 ADR-006 状态机，不旁路。
