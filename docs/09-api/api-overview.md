...
| Agent Evaluation Backend | `/api/execution/results/:id/evaluations`、rule evaluation、`/api/execution/evaluations/regression-run`、analytics、low-quality list、`/api/execution/evaluations/model-comparison` tag-based 模型对比、`/api/execution/evaluations/cost-attribution` provider runtime metadata 成本归因校准、只读 dashboard UI | LLM judge、billing-grade 成本结算、跨模型回归评测编排 |
