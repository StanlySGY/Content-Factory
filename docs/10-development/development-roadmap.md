...
| Sprint | 目标 | 最小可运行结果 |
| --- | --- | --- |
| Sprint 1 | 建立基础工程、数据库与核心领域模型 | 可创建项目、用户、内容任务，查看任务列表 |
| Sprint 2 | 建立工作流与资产版本闭环 | 可启动工作流、推进阶段、保存阶段资产版本 |
| Sprint 3 | 建立审核、Dashboard 与编辑体验 | 可审核、退回、查看 Dashboard 和文章编辑页 |
| Sprint 4 | 建立 Agent / MCP / 公众号工作台 MVP 壳层 | 可配置 Agent / MCP，查看日志，完成发布准备记录 |
...
- `publish_records` 可先用 `content_assets` + `audit_events` 表达，后续独立建表。
...
- 使用 `AppShell`、`SidebarNav`、`TopBar`、`ContextPanel` 建立统一布局。
- 页面不直接承载业务规则。
- 高风险动作必须阻断式确认。
- 状态不可只用颜色表达。
- 长列表预留分页或虚拟滚动。
