...
| RBAC Management UI | 已完成 | Web `/rbac` 支持 organization member 添加、角色更新、停用，以及默认项目 membership 授权/撤销；成员和 membership 变更写入审计链；不接 auth/session、不做全局业务 API enforcement、不替代角色变更审批策略 |
