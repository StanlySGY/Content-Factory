...
| Knowledge/RAG Backend | `/api/knowledge/sources`、entries、archive/restore、keyword search、task candidates、`/api/knowledge/embedding-readiness`、`/api/knowledge/vector-search`、本地 deterministic embedding snapshot、本地 vector retrieval、只读 candidate review UI | 生产级 vector index、LLM rerank、context pack 自动刷新 |
